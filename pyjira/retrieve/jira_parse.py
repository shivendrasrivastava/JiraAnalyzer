...
def parse(url, prj_name, jira, dirname, user, pwd):
	...
	retrieve(url, total_no_of_issues, query_string, jira, dirname, user, pwd)
...
def writeToFile(dirname, json_data, key):
...
def retrieve(url, total_no_of_issues, query_string, jira, dirname, user, pwd):
	issues = None
	if total_no_of_issues > 1000:
		...
		while no_of_issues > 0:
			...
			if issues is not None:
				for key in issues:
					request_data = requests.get(get_rest_api_url(url, key), auth=(user, pwd))
					json_data = json.loads(request_data.text)
					#print "Writing file for issue ", key
					writeToFile(dirname, json_data, key)
	else:
...
def get_rest_api_url(url, key):
	return url + "/rest/api/latest/issue/" + str(key)
